<template>
    <div class="popup-layer-container" :class="{ show: visible, animation: isAnimation }" :style="{ zIndex:autoIndex?getMaxZIndex():zIndex }" :id="id">
        <slot></slot>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
         ...
         */
        guid() {
            return Array.from({ length: 8 }, (_, i) => (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1) + ([1, 2, 3, 4].includes(i) ? '-' : '')).join('')
        },
        /**
         * @description: 打开弹层
         * 1. 打开弹出层
...
.popup-layer-container.animation {
    transition: transform 0.3s;
}
.popup-layer-container.show {
